...
function chooseRender(text, filePath) {
  ...
  console.log(`chooseRender filePath = ${filePath}`);

  let filePathExt = path.extname(filePath);
  if ( asciidocExtensions.includes( filePathExt)) {
    console.log(`chooseRender fn render isAsciidoctor = ${isAsciidoctor}`)
    isAsciidoctor = true;
  } else {
    isAsciidoctor = false;
  }
  console.log('chooseRender typeof isAsciidoctor =');  let typeofIsAsciidoctor = typeof isAsciidoctor;
  console.log('chooseRender ' + typeof( isAsciidoctor)); console.log(` chooseRender typeofIsAsciidoctor = ${typeofIsAsciidoctor}`);
  if (isAsciidoctor == true) { return renderAsciidoctor (text, filePath) }
  ...
  // if text inclused :choosePandoc.. and a legal mdFlavours
  // OR first 10 lines incluses that
  console.log("chooseRender about to run checkMdFlavour")
  console.log(`checkMdFlavour=${checkMdFlavour( text, filePath )}`)
  console.log("chooseRender just run checkMdFlavour")
  // if ( text.includes(":choosePandocMdFlavour:" ) ) {
  //   console.log( "chooseRender contains :choosePandocMdFlavour:" );
  //   console.log( `chooseRender text = \n ${text}`)
  //   return renderPandocMdFlavour( text, filePath );  // NB NB return essential here

  //return [ isMdFlavour, mdFlavourFound , isKnownFlavour ]  // false,false,false   true,markdownX,false   true,markdown,true
  //return [ isMdFlavour, Boolean(mdFlavourFound) ? mdFlavourFound : " " , isKnownFlavour ] // false, ,false  true,markdownX,false
  //return [ isMdFlavour, !!mdFlavourFound ? mdFlavourFound : "notfound" , isKnownFlavour ] // false,notfound,false

  //let fromCheckMdFlavour = checkMdFlavour( text, filePath )
  let [ isMdFlavourRet, mdFlavourFoundRet , isKnownFlavourRet ] = checkMdFlavour( text, filePath )
  console.log( `fromCheckMdFlavour isMdFlavourRet= ${isMdFlavourRet}`)
  console.log( `fromCheckMdFlavour mdFlavourFoundRet= ${mdFlavourFoundRet}`)
  console.log( `fromCheckMdFlavour isKnownFlavourRet= ${isKnownFlavourRet}`)
  if ( !(isKnownFlavourRet) && isMdFlavourRet) {alert( `fromCheckMdFlavour isKnownFlavourRet= ${mdFlavourFoundRet} NOT known.`) }
  if ( !!(isMdFlavourRet) && !!(isKnownFlavourRet) ) {
  return   renderPandocMdFlavour( text, filePath, mdFlavourFoundRet )
  }
...
// define mdFlavours, split first NLines, search for mdFlavour
function checkMdFlavour( text, filePath) {
  const nLinesSplitOff = 20 // number of lines searched for pandocMdFlavour string
  const mdFlavours = ['commonmark', 'commonmark_x', 'markdown', 'markdown_mmd', 'markdown_phpextra', 'markdown_strict', 'gfm', 'markdown_github'];
  const mdFlavoursFull = mdFlavours.map( i => ":choosePandocMdFlavour:" + i + ":");
  console.log(`checkMdFlavour l260 mdFlavoursFull=${mdFlavoursFull}`);
  // split off 1st nLinesSplitOff lines in arrayOfLines
  let arrayOfLines = text.split(/\r?\n/, nLinesSplitOff);
  console.log(`checkMdFlavour l263 nLinesSplitOff=${nLinesSplitOff}  arrayOfLines=${arrayOfLines}`);
  let mdFlavourFound = "";
  const regex = /(?<=\:choosePandocMdFlavour\:).*(?=\:)/;

  for(let j=0; j < arrayOfLines.length; j++ ) {
    //result = checkRegexSimple( regex, line.toString() )
    if ((Boolean(arrayOfLines[j])) !== null) {
      console.log(`checkMdFlavour l273 arrayOfLines ${j}=${arrayOfLines[j]} `)
      result = arrayOfLines[j].toString().match( regex )
      console.log(`checkMdFlavour l275  result=${result} `)

    } else {
      result = false
      console.log(`checkMdFlavour l279 MATCH NOT FOUND result=${result} `)
    }
    if ( result ) { break }
  }
  console.log(`checkMdFlavour l288 result=${result}` )
  console.log(`checkMdFlavour l289 mdFlavours=${mdFlavours}`)
  if (result) {console.log(`checkMdFlavour l276 MATCH FOUND result=${result} `) }
  mdFlavourFound = result
  let isMdFlavour = Boolean( result )

  let isKnownFlavour
  if (Boolean(result)) { mdFlavourFound = result } else {mdFlavourFound = false }
  //return Boolean(result) ? result : false
  // if true return result if false return false.

  if (Boolean(mdFlavourFound)) {
    if ( mdFlavours.includes(mdFlavourFound.toString()) ) {
      console.log(`checkMdFlavour l294 MATCH FOUND in mdFlavours`)
     // return mdFlavourFound
      isKnownFlavour = true
    } else {
      console.log(`checkMdFlavour l282 MATCH NOT FOUND in mdFlavours isKnownFlavour=${isKnownFlavour}`)
      // return "NOT in mdFlavours ${mdFlavourFound}"
      isKnownFlavour = false
    }

  } //else {
    //isKnownFlavour = false  // this is not right should not be here.
  //}// END if (result)
  //return result
 //return mdFlavourFound
 console.log(`checkMdFlavour [ isMdFlavour, mdFlavourFound , isKnownFlavour= ` + [ isMdFlavour, mdFlavourFound , isKnownFlavour ] )
  return [ isMdFlavour, mdFlavourFound , isKnownFlavour ]  // false,false,false   true,markdownX,false   true,markdown,true
  //return [ isMdFlavour, Boolean(mdFlavourFound) ? mdFlavourFound : " " , isKnownFlavour ] // false, ,false  true,markdownX,false
  //return [ isMdFlavour, !!mdFlavourFound ? mdFlavourFound : "notfound" , isKnownFlavour ] // false,notfound,false
} //END checkMdFlavour()

var renderPandocMdFlavour =  function (text, filePath, mdFlavourFound) {
    ...
    const inputFilePath = filePath;  //'/media/AcerWinData/github_from_home

    const mdFlavours = ['markdown','markdown_strict', 'markdown_phpextra',  'markdown_mmd', 'commonmark', 'commonmark_x',   'markdown_gfm', 'markdown_github']; // , 'rst'];
    ...
    // :choosePandocMdFlavour:markdown_github: (deprecated GitHub-Flavored Markdown)mdFlavoursFull=:choosePandocMdFlavour:commonmark:,:choosePandocMdFlavour:commonmark_x:,:choosePandocMdFlavour:markdown:,:choosePandocMdFlavour:markdown_mmd:,:choosePandocMdFlavour:markdown_phpextra:,:choosePandocMdFlavour:markdown_strict:


    //const mdFlavoursFull = mdFlavours.map( i => ":choosePandocMdFlavour:" + i + ":");
    //console.log(`efsg mdFlavoursFull=${mdFlavoursFull}`);

    // split off 1st 10 lines in arrayOfLines
    let nLinesSplitOff = 20
    let arrayOfLines = text.split(/\r?\n/, nLinesSplitOff); console.log("wdgt  arrayOfLines =" + arrayOfLines);
    ...
    //   //result = checkRegexSimple( regex, line.toString() )
    //    result = line.toString().match( regex )
    //
    //   if (result) {console.log(`sfgj MATCH FOUND result=${result} line=${line}`)
    //     mdFlavourFound = result
    //   } else {console.log `sfgj NO MATCH line=${line}`}
    // })
    console.log(`mdFlavourFound=${mdFlavourFound}`)
    flavour = mdFlavourFound

    let outputPath = `${filePath}_${flavour}.html`; console.log(`ASFE outputPath = = ${outputPath}`)
    // NB NB --standalone must be removed.
    const commandlineCommand = `${pandocBin} --sandbox  -f ${flavour} -t html5 --metadata title="${flavour} command" --verbose  -o - ${inputFilePath}`
    const fileOutputCommand = `${pandocBin} --sandbox  -f ${flavour} -t html5 --metadata title="${flavour} command" --verbose -o ${outputPath} ${inputFilePath}`

    //let command = fileOutputCommand
    console.log(`klop file output command =${fileOutputCommand}`)
    textToHTMLFile = cp.execSync( fileOutputCommand );

    //command = commandlineCommand
    console.log(`klop commandline command =${commandlineCommand}`)
    textToHTMLCommandLine = cp.execSync( commandlineCommand );

     console.log(`dhkp ianschange <li> textToHTMLCommandLine >/li> = \n <li> ${textToHTMLCommandLine}</li>\n `);
     //return `<li> ${text}</li>\n`;
     html = textToHTMLCommandLine.toString(); // needed or ERR trim not a fn below
     /* ianschange removing template seems to make no difference */
     let now = new Date();
     html =  `<h3>pandoc mdFlavourFound=${flavour}</h3>` +
              `<p>commandlineCommand=${commandlineCommand}</p>` +
