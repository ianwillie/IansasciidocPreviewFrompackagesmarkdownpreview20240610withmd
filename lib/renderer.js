...
var renderAsciidoctor =  function (text, filePath  , backend = 'html5'  ) {
  ...
  var renderPandocMdFlavour =  function (text, filePath) {
    ...
    function run() {
      ...
      let filePath = '/media/AcerWinData/github_from_home/IansasciidocPreviewFrompackagesmarkdownpreview20240610withmd/TEST_Adoc_MD_Files_Etc/README_SHORT.md';
      let outputPath = './pandoc_README_SHORT.html';

      const mdFlavours = ['commonmark', 'commonmark_x', 'markdown', 'markdown_mmd', 'markdown_phpextra', '?<error' ,'markdown_strict']; // , 'rst'];
      ...
      //   // const command = `pandoc --sandbox  --standalone ${flavour} -t html5 --metadata title="${flavour}"  -o ${outputPath} ${filePath}`; //metadata <head><title>...</title></head>
       ...
       const command = `${pandocBin} --sandbox  --standalone ${flavour} -t html5 --metadata title="${element}: ${mdFlavours[element]}"  -o ${outputPath} ${filePath}` ;
        ...
        console.log('using pandoc converting');

        //Logger.debug(`exec command stdout: ${stdout.toString()}`);
       // console.log(`exec command stdout: ${stdout.toString()}`);

        cp.exec(`/usr/bin/falkon ${outputPath} `);
    ...
    console.log('Prog run ENDS');
     ...
     return fragment // NB NB fragment NEEDS TO BE DEFINED There
