...
function chooseRender(text, filePath) {
  ...
  if ( text.includes(":choosePandocMdFlavour:" ) ) {
    ...
    // html =  `<h1>:Ians choosePandocMdFlavour:commonmark:</h1>` + html
...
function checkMdFlavour( text, filePath) {
  ...
  const mdFlavours = ['commonmark', 'commonmark_x', 'markdown', 'markdown_mmd', 'markdown_phpextra', 'markdown_strict'];
  ...
  for(let i=0; i < arrayOfLines.length; i++ ) {
    ...
    if ((Boolean(arrayOfLines[i])) !== null) {
      console.log(`checkMdFlavour l273 arrayOfLines ${i}=${arrayOfLines[i]} `)
      result = arrayOfLines[i].toString().match( regex )
...
var renderPandocMdFlavour =  function (text, filePath) {
    ...
    const mdFlavours = ['markdown','markdown_strict', 'markdown_phpextra',  'markdown_mmd', 'commonmark', 'commonmark_x',   'gfm', 'markdown_github']; // , 'rst'];
