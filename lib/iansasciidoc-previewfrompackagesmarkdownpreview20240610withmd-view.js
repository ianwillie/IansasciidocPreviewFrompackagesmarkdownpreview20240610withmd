...
module.exports = class IansasciidocPreviewfrompackagesmarkdownpreview20240610withmdView {
  static deserialize(params) {
    return new IansasciidocPreviewfrompackagesmarkdownpreview20240610withmdView(params)
  }
  ...
  constructor({ editorId, filePath }) {
    ...
    this.element.classList.add('iansasciidoc-previewfrompackagesmarkdownpreview20240610withmd')
  ...
  serialize() {
    return {
      deserializer: 'IansasciidocPreviewfrompackagesmarkdownpreview20240610withmdView',
  ...
  copy() {
    return new IansasciidocPreviewfrompackagesmarkdownpreview20240610withmdView({
      editorId: this.editorId,
      filePath: this.getPath() != null ? this.getPath() : this.filePath
    })
  }
  ...
  handleEvents() {
    ...
    atom.commands.add(this.element, {
      ...
      'iansasciidoc-previewfrompackagesmarkdownpreview20240610withmd:select-all': () => {
        this.selectAll()
      },
      'iansasciidoc-previewfrompackagesmarkdownpreview20240610withmd:zoom-in': () => {
        const zoomLevel = parseFloat(getComputedStyle(this.element).zoom)
        this.element.style.zoom = zoomLevel + 0.1
      },
      'iansasciidoc-previewfrompackagesmarkdownpreview20240610withmd:zoom-out': () => {
        const zoomLevel = parseFloat(getComputedStyle(this.element).zoom)
        this.element.style.zoom = zoomLevel - 0.1
      },
      'iansasciidoc-previewfrompackagesmarkdownpreview20240610withmd:reset-zoom': () => {
    ...
    if (this.file) {
      ...
      this.disposables.add(
        this.editor.getBuffer().onDidStopChanging(function () {
          if (atom.config.get('iansasciidoc-previewfrompackagesmarkdownpreview20240610withmd.liveUpdate')) {
            changeHandler()
          }
        })
      )
    ...
    this.disposables.add(
      atom.config.onDidChange(
        'iansasciidoc-previewfrompackagesmarkdownpreview20240610withmd.breakOnSingleNewline',
        changeHandler
      )
    )

    this.disposables.add(
      atom.config.observe('iansasciidoc-previewfrompackagesmarkdownpreview20240610withmd.gitHubStyleMode', gitHubStyleMode => {
    ...
    this.disposables.add(
      atom.config.observe('iansasciidoc-previewfrompackagesmarkdownpreview20240610withmd.useGitHubStyle', useGitHubStyle => {
  ...
  getTitle() {
    if (this.file != null && this.getPath() != null) {
      ...
      return 'Iansasciidoc Previewfrompackagesmarkdownpreview20240610withmd'
    }
  }
  ...
  getURI() {
    if (this.file != null) {
      return `iansasciidoc-previewfrompackagesmarkdownpreview20240610withmd://${this.getPath()}`
    } else {
      return `iansasciidoc-previewfrompackagesmarkdownpreview20240610withmd://editor/${this.editorId}`
    }
  }
  ...
  getIansasciidocPreviewfrompackagesmarkdownpreview20240610withmdCSS() {
    const markdownPreviewRules = []
    const ruleRegExp = /\.iansasciidoc-previewfrompackagesmarkdownpreview20240610withmd/
    const cssUrlRegExp = /url\(atom:\/\/iansasciidoc-previewfrompackagesmarkdownpreview20240610withmd\/assets\/(.*)\)/
  ...
  async saveAs(htmlFilePath) {
    if (this.loading) {
      atom.notifications.addWarning(
        'Please wait until the Iansasciidoc Previewfrompackagesmarkdownpreview20240610withmd has finished loading before saving'
      )
      return
    }
...
<html>
  <head>
      <meta charset="utf-8" />
      <title>${title}</title>
      <style>${this.getIansasciidocPreviewfrompackagesmarkdownpreview20240610withmdCSS()}</style>
  </head>
  <body class='iansasciidoc-previewfrompackagesmarkdownpreview20240610withmd' data-use-github-style="${this.gitHubStyleMode}">${htmlBody}</body>
